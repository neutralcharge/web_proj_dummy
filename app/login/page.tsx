  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (role === "doctor") {
          router.push("/appointment-view") // Doctors always go to appointment view
        } else {
          router.push("/start-consultation") // Patients go to consultation
        }
      } else {
        ...
        <div ref={formRef} className="login-content">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full h-11 bg-blue-600 hover:bg-blue-700 text-white transition-all duration-200 rounded-lg"
                disabled={isLoading}
              >
                {isLoading ? (
